...
<section>
    ...
    <h2>LjubavniBrodolomi</h2>

    <?php
    include 'connect.php';
    define('UPLPATH', 'img/');

    $query = "SELECT * FROM clanak WHERE arhiva=0 AND kategorija='LjubavniBrodolomi' LIMIT 4";
...
<section>
    ...
    $query = "SELECT * FROM clanak WHERE arhiva=0 AND kategorija='KogaBriga' LIMIT 4";
    $result = mysqli_query($dbc, $query);
    $i=0;
    while($row = mysqli_fetch_array($result)) {
        echo "<article class='clanak_indx'>";
        echo '<a href="clanak.php?id=' . $row['id'] . '">';
        echo "<div class='article_img'>";
        echo '<img src="' . "img/" . $row['slika'] . '" alt="' . $row['slika'] . '"/>';
        echo "</div>";
        echo '<h3>' . $row['naslov'] . '</h3>';
        echo "</a>";
        echo "</article>";
